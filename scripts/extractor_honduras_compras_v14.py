#!/usr/bin/env python3
"""
Extractor v14 - v5 mejorado con corrección de expediente y debugging
Corrige extracción de expediente que estaba incluyendo todo el contenido de la celda
"""
...
class SICCExtractorV14:
    """Extractor v14 - Corrigiendo expediente y debuggeando Cuerpo de Bomberos"""
    ...
    def _limpiar_expediente(self, expediente_texto: str) -> str:
        """Limpia el expediente extrayendo solo el número de expediente"""
        lines = expediente_texto.split('\n')

        for line in lines:
            line = line.strip()
            if line and not any(label in line for label in ['Entidad:', 'Unidad', 'Objeto', 'Adquisición']):
                return line

        return expediente_texto.split('\n')[0].strip() if expediente_texto else ""
    ...
    def extraer_licitaciones_completo(self) -> List[Dict[str, Any]]:
            ...
            try:
                for inst_code, (inst_nombre, inst_sigla) in self.institution_map.items():
                    ...
                    try:
                        ...
                        instituciones_procesadas.append((inst_sigla, nuevos))

                    except Exception as e:
                        print(f"❌", flush=True)
                        instituciones_procesadas.append((inst_sigla, 0))
                        continue
        ...
        for sigla, count in instituciones_procesadas:
            print(f"      - {sigla}: {count}")
    ...
    def _extraer_procesos(self, page, tipo: str = "licitacion") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for tabla in tablas:
                ...
                if len(filas) > 2:
                    # Verificar si es la tabla de resultados: debe tener celdas de datos con 5 columnas
                    fila_datos = filas[1] if len(filas) > 1 else None
                    if fila_datos:
                        celdas = fila_datos.query_selector_all('td')
                        if len(celdas) == 5:  # Tabla de resultados tiene 5 columnas
                            tabla_procesos = tabla
                            break

            if not tabla_procesos:
                # Fallback: buscar por header text, pero ser más específico
                for tabla in tablas:
                    filas = tabla.query_selector_all('tr')
                    if len(filas) > 2:
                        primera_fila = filas[0]
                        header_text = primera_fila.text_content()
                        if "Adquisición" in header_text and "Expediente" not in header_text and len(filas) > 20:
                            tabla_procesos = tabla
                            break
            ...
            for fila in datos_filas:
                try:
                    ...
                    expediente_texto = celdas[0].inner_text().strip() if len(celdas) > 0 else ""
                    etapa = celdas[1].inner_text().strip() if len(celdas) > 1 else ""
                    modalidad = celdas[2].inner_text().strip() if len(celdas) > 2 else ""
                    vigencia_texto = celdas[3].inner_text().strip() if len(celdas) > 3 else ""
                    ...
                    # CORREGIR: Limpiar expediente para extraer solo el número
                    expediente = self._limpiar_expediente(expediente_texto)
                    expediente = expediente.replace("Expediente:", "").strip()
...
def main():
    ...
    datos_lic = {
        "metadata": {
            ...
            "metodo_extraccion": "playwright-por-institucion-pagination-v14"
        },
